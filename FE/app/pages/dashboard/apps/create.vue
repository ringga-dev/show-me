...
import {useToast} from '#imports' // Nuxt composable (Nuxt UI / nuxt-ui)
...
/* ---------------- Local UI types ---------------- */
/** Template memakai name & image untuk iklan */
type UiAd = {
  title?: string | null
  image?: string | null
...
/* ---------------- Source form ---------------- */
/** Jika sudah punya data dari store, silakan ganti inisialisasi ini */
...
// snapshot untuk reset
const initialApp = deepClone(appForm)
...
const handleSubmit = async () => {
  ...
  try {
    // contoh panggilan API (sesuaikan endpoint & typing)
    const res = await apps.addApp(appForm)
    console.log(res)
    if (res?.status?.toUpperCase() === "SUCCESS") {
      go('/dashboard/apps/list')
      toast.add?.({title: 'Saved', description: 'Changes have been saved.', color: 'success'})
    } else {
      const error = res?.message ?? ['Save failed']
      toast.add?.({title: 'Saved', description: error[0], color: 'warning'})
    }
...
const handleReset = () => {
  // reset seluruh form dari snapshot awal
