...
// numberLike: terima number/string → number
...
const schema = v.object({
  ...
  dateCreated: v.optional(v.string()), // <input type="date">
...
/** "YYYY-MM-DD" untuk prefill input[type=date] */
...
/** ke format LocalDateTime ISO tanpa zona */
...
async function onSubmit(_e: FormSubmitEvent<Schema>) {
  try {
    ...
    const res = await store.addPortfolioProject(payload)
    const ok  = (res?.status ?? 'OK') === 'OK'
    if (ok) {
      toast.add({ title: 'Saved', description: 'Project created.' })
...
<template>
  <div class="space-y-4">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      <h1 class="text-2xl font-semibold text-slate-800 dark:text-white">Create Project</h1>
    ...
    <UCard class="sticky top-2 z-10 backdrop-blur bg-white/70 dark:bg-gray-900/70 border border-gray-200/60 dark:border-gray-800/60">
      <div class="flex items-center justify-between gap-3">
        <div class="text-sm text-slate-500 dark:text-slate-400">Isi data proyek lalu simpan. Field bertanda wajib.</div>
        <div class="flex gap-2">
          <UButton type="submit" form="project-form" :loading="submitting" icon="i-heroicons-check-circle">Save</UButton>
    ...
    <!-- Form -->
    <UForm id="project-form" :schema="schema" :state="form" class="space-y-4" @submit="onSubmit">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <!-- LEFT -->
        <UCard class="space-y-4">
          <div class="grid grid-cols-1 gap-4">
            <UFormField label="Title" name="title">
              <UInput v-model="form.title" placeholder="BatamFast App" class="w-full"
            ...
            <UFormField label="GitHub URL" name="githubUrl">
              <UInput v-model="form.githubUrl" placeholder="https://github.com/username/repo" type="url" class="w-full"
                      :ui="{ base:'bg-gray-50 border border-gray-300 text-gray-900 rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white' }"/>
            </UFormField>

            <UFormField label="Demo URL" name="demoUrl">
              <UInput v-model="form.demoUrl" placeholder="https://your-demo.com" type="url" class="w-full"
                      :ui="{ base:'bg-gray-50 border border-gray-300 text-gray-900 rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white' }"/>
            </UFormField>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <UFormField label="Release Version" name="releseVersion">
                <UInput v-model="form.releseVersion" placeholder="4.3.1" class="w-full"
                        :ui="{ base:'bg-gray-50 border border-gray-300 text-gray-900 rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white' }"/>
              </UFormField>
              <UFormField label="Date Created" name="dateCreated">
                <UInput v-model="form.dateCreated" type="date" class="w-full"
                        :ui="{ base:'bg-gray-50 border border-gray-300 text-gray-900 rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white' }"/>
              </UFormField>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <UFormField label="User Request" name="userRequest">
                <UInput v-model="form.userRequest" type="number" min="0" step="1" class="w-full"
                        :ui="{ base:'bg-gray-50 border border-gray-300 text-gray-900 rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white' }"/>
              </UFormField>
              ...
              <UFormField label="Rating (0–5)" name="rating">
                <div class="space-y-2">
                  <div class="flex items-center gap-1.5">
                    <button v-for="i in 5" :key="i" type="button" class="p-1 rounded hover:bg-gray-100 dark:hover:bg-gray-800" @click="setRating(i)">
                  ...
                  <input v-model="form.rating" type="range" min="0" max="5" step="0.5"
        ...
        <UCard class="space-y-4">
          ...
          <div>
            <div class="flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-t-xl">
              ...
              <UButton size="xs" color="red" variant="ghost" icon="i-heroicons-x-mark" @click="clearImage" />
            </div>
            ...
            <UFormField label="Image URL" name="imageUrl" class="mt-3">
              <div class="flex gap-2">
                <UInput v-model="form.imageUrl" placeholder="https://…" type="url" class="w-full"
                        :ui="{ base:'bg-gray-50 border border-gray-300 text-gray-900 rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white' }"/>
                <UButton v-if="form.imageUrl" color="gray" variant="soft" icon="i-heroicons-arrow-top-right-on-square"
